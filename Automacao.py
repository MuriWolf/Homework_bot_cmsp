...
#     entrar_html("question-choice",871, 532)
...
while True:
    ...
    # copiar a perguta
    pergunta = pyautogui.confirm("copie a questao e aperte em pronto", "continuar")
    if pergunta == "OK":
        pyautogui.hotkey("ctrl", "t")
        time.sleep(1)
        colar()
    ...
    elif pergunta == "Cancel":
        saida = pyautogui.confirm("Deseja sair?", "Confirmação de saída")
        if saida == "OK":
            break
        continue

    confirmacao = pyautogui.confirm("Clique em ok quando enviar a resposta", "Proxima questao")
